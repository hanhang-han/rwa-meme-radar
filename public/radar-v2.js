...
(() => {
  ...
  function overview(){
    ...
    return `<div class="hero x-hero"><div class="v2-hero-copy"><span class="eyebrow">STOCKSMEME / RELATIONSHIP INTELLIGENCE</span><h2>${tr('发现股票相关 Meme，查看链上配对证据。','Find stock-related memes with on-chain pair evidence.')} ${help('overview')}</h2></div><form id="v2Search"><input name="q" placeholder="${tr('搜索股票、币名或合约地址','Search stocks, tokens or contract addresses')}" aria-label="${tr('全局搜索','Global search')}" required><button>${tr('搜索','Search')}</button></form></div>
      <section class="panel x-ai"><h2>${tr('今日变化摘要','Today changes')}</h2><p class="hint">${tr('由 DeepSeek 基于本站已采集字段生成，只解读已有数据，不预测价格。','Generated by DeepSeek from collected fields only; no price forecasts.')}</p><div class="v2-ai-text" id="v2Briefing"></div></section><div class="kpis">${card(tip('priority',tr('达到流动性门槛的 Meme','Memes above the liquidity floor')),num(m.actionableAssets),tr('已核验且新鲜池流动性 ≥ $1,000','Verified with fresh pool liquidity ≥ $1,000'),'#meme?filter=related')}${card(tip('evidence',tr('已确认的股票配对池','Confirmed stock pair pools')),num(m.verifiedPools),tr('按链与池地址去重','Deduplicated by chain and pool'),'#pair')}${card(tip('liquidity',tr('已收录股票配对池流动性','Listed pair-pool liquidity')),usd(m.pairedLiquidityUsd),`${num(cov.valued)} / ${num(cov.total)} ${tr('池有新鲜估值','pools with fresh valuations')} · ${tr('覆盖数量变化不等于资金进出','coverage changes are not flows')}`)}${card(tip('leads',tr('24h 新增配对','New pairs in 24h')),num(m.newRelations24h),tr('按首次发现时间统计','Counted by first detection'),'#events')}</div>
  ...
  function memePage(){
    ...
    const row=(a,extra='',child=false)=>`<tr class="${child?'v2-member-row':''}"><td>${logo(a)}<a href="${detailLink(a)}"><strong>${esc(a.symbol)}</strong></a><small>${chainNames[chain(a)]} · ${short(a.token)} · ${esc(a.provider??'OKX')}</small>${extra}</td><td>${(()=>{const rr=relations(a);const shown=rr.slice(0,2).map(r=>`<a href="${pairLink(r)}">${esc(r.ticker)} · ${status(r)}</a>`).join('<br>');const more=rr.length>2?`<br><small title="${esc(rr.map(r=>r.ticker).join(', '))}">+${rr.length-2}</small>`:'';return shown+more||tr('待核实','Pending');})()}</td><td>${field(a,'liquidity',true)}</td><td>${field(a,'volume24h',true)}</td><td>${field(a,'txs24h')}<small>${tr('买 / 卖','Buys / sells')}: ${field(a,'buys24h')} / ${field(a,'sells24h')}</small></td><td>${field(a,'price',true)}</td><td>${field(a,'holders')}</td><td>${age(a.fieldTimes?.price??a.updatedAt)}<small>${tr('本站首次收录','First listed')} ${date(a.firstSeen)}</small></td></tr>`;
